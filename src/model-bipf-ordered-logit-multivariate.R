...
## Sample the posterior
fit_ord_log_multivariate <- 
  stan(model_code = model_ord_log_multivariate, 
       data = data_list, 
       chains = 4,
       warmup = 2000, 
       iter = 4000, 
...
fit_ord_log_multivariate %>% tidybayes::summarise_draws() %>% print(n = 700)
...
# RECOVER DATA TYPES
fit_ord_log_multivariate <- fit_ord_log_multivariate %>% recover_types(data_list)

# EXTRACT DRAWS
draws_ord_log_multivariate <-
  fit_ord_log_multivariate %>%
  recover_types(data_list) %>%
  spread_draws(beta[1:7], cutpoints[1:5]) %>% 
...
# Rank Plots
bayesplot::mcmc_rank_hist(fit_ord_log_multivariate, 
                          pars = c('beta[1]', 
                                   'beta[2]',
                                   'beta[3]',
                                   'beta[4]',
