...
## (roughly the min and max of mios_total in data_baked_3 -- in SD not original units)
## and every combination of the dummy-coded covariates.

#min(data_baked_3$mios_total)
#max(data_baked_3$mios_total)
#min(data_baked_3$pcl_total, na.rm = TRUE) # there are missing values
#max(data_baked_3$pcl_total, na.rm = TRUE)
data_baked_3 %>% count(pcl_total) %>% print(n = 100)

data_sim_3 <-
  expand_grid(
    mios_total = seq(-2, 2, by = .333), # Range
    pcl_total = seq(-2, 2.5, by = .333), # Range
  ...
  mutate(
    ...
    mios_total_og = mios_total * sd(data_3$mios_total) + mean(data_3$mios_total)
  ) %>% 
...
linpred_prior_3 <- 
  posterior_linpred(model_3_hurdle_prior, 
                    newdata = data_sim_3, 
                    dpar = "mu", 
                    transform = FALSE)

# Summarize across draws to get a single estimate per row (mean of the posterior)
data_sim_3$log_mu <- colMeans(linpred_prior_3)

# compute posterior mean on outcome scale (not the log scale)
data_sim_3$mu <- exp(data_sim_3$log_mu)


# Plot linear predictor (Response Scale) ---------------------------------------
plot_prior_predictive_3 <-
ggplot(data_sim_3, aes(x = mios_total_og, y = mu)) +
...
## Print to console ------------------------------------------------------------
plot_prior_predictive_3 %>% print()


## Save to file ----------------------------------------------------------------
ggsave(plot = plot_prior_predictive_3, file = here::here("output/plot-prior-predictive-3.jpg"), width = 6, height = 4)


# Plot prior draws for coefficients  -------------------------------------------
plot_prior_draws_3 <-
  as_draws_df(model_3_hurdle_prior) %>%
