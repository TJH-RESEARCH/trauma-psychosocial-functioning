...
# Get Draws ---------------------------------------------------------------
draws_hurdle_3 <- model_3_hurdle %>% tidy_draws()


# Check Rhat and Effective Sample Size -----------------------------------------
draws_hurdle_3 %>% count(divergent__)
draws_hurdle_3 %>% count(n_leapfrog__)
draws_hurdle_3 %>% count(stepsize__)
draws_hurdle_3 %>% ggplot(aes(accept_stat__)) + geom_density()
bayestestR::effective_sample(model_3_hurdle)
bayestestR::diagnostic_draws(model_3_hurdle)


broom.mixed::tidy(model_3_hurdle) %>% count(term) %>% print(n = 300)
