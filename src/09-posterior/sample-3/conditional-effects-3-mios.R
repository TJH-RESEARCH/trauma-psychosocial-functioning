...
# Combined Model
plot_conditional_effects_3_mios <- 
  plot(conditional_effects(model_3_hurdle), plot = FALSE)[[2]]$data %>%  
...
plot_conditional_effects_3_mu_mios <- 
  plot(conditional_effects(model_3_hurdle, dpar = "mu"), plot = FALSE)[[2]]$data %>% 
  mutate(mios_total = effect1__ * sd(data_3$mios_total) + mean(data_3$mios_total)) %>% 
  rename(bifp_pred = estimate__) %>% 
  ggplot(aes(x = mios_total, y = bifp_pred)) +
  geom_ribbon(aes(ymin = lower__, ymax = upper__), alpha = 0.3, color = NA) +
...
plot_conditional_effects_3_hu_mios <- 
  plot(conditional_effects(model_3_hurdle, dpar = "hu"), plot = FALSE)[[2]]$data %>% 
  mutate(mios_total = effect1__ * sd(data_3$mios_total) + mean(data_3$mios_total)) %>% 
  rename(bifp_pred = estimate__) %>% 
  ggplot(aes(x = mios_total, y = bifp_pred)) +
  geom_ribbon(aes(ymin = lower__, ymax = upper__), alpha = 0.3, color = NA) +
...
# Combine the plots
plot_conditional_effects_3_combo_mios <-
  (plot_conditional_effects_3_mios / plot_conditional_effects_3_mu_mios / plot_conditional_effects_3_hu_mios) +
...
ggsave(plot = plot_conditional_effects_3_combo_mios, file = here::here("output/plot-conditional-effects-3-mios.jpg"), width = 6, height = 8)
