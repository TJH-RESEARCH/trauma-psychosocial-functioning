
# Combined Model
(plot_conditional_effects_2 <- 
  plot(conditional_effects(model_2_hurdle), plot = FALSE)[[1]]$data %>%  
  # transform mios from SD to original units
  mutate(mios_total = effect1__ * sd(data_2$mios_total) + mean(data_2$mios_total)) %>% 
  rename(bifp_pred = estimate__) %>% 
  ggplot(aes(x = mios_total, y = bifp_pred)) +
  geom_ribbon(aes(ymin = lower__, ymax = upper__), alpha = 0.3, color = NA) +
  geom_line(size = 1, color = colors_tam[1]) +
  scale_x_continuous(limits = c(-1,60), breaks = seq(0, 60, 10)) +
  scale_y_continuous(limits = c(0,170), breaks = seq(0, 160, 40)) +
  labs(x = "Moral Injury Symptoms (MIOS)", 
       y = "Predicted Dysfunction (bIPF)",
       subtitle = "Combined parts of the model (Hurdle + Gamma)") +
  theme_scatter
)
# Non-Zero Gamma Model
plot_conditional_effects_2_mu <- 
  plot(conditional_effects(model_2_hurdle, dpar = "mu"), plot = FALSE)[[1]]$data %>% 
  mutate(mios_total = effect1__ * sd(data_2$mios_total) + mean(data_2$mios_total)) %>% 
  rename(bifp_pred = estimate__) %>% 
  ggplot(aes(x = mios_total, y = bifp_pred)) +
  geom_ribbon(aes(ymin = lower__, ymax = upper__), alpha = 0.3, color = NA) +
  geom_line(size = 1, color = colors_tam[3]) +
  scale_x_continuous(limits = c(-1,60), breaks = seq(0, 60, 10)) +
  scale_y_continuous(limits = c(0,170), breaks = seq(0, 160, 40)) +
  labs(x = "Moral Injury Symptoms (MIOS)", 
       y = "Predicted Dysfunction (bIPF)",
       subtitle = "Gamma part of the model") +
  theme_scatter


# Zero-Process Logistic Model
plot_conditional_effects_2_hu <- 
  plot(conditional_effects(model_2_hurdle, dpar = "hu"), plot = FALSE)[[1]]$data %>% 
  mutate(mios_total = effect1__ * sd(data_2$mios_total) + mean(data_2$mios_total)) %>% 
  rename(bifp_pred = estimate__) %>% 
  ggplot(aes(x = mios_total, y = bifp_pred)) +
  geom_ribbon(aes(ymin = lower__, ymax = upper__), alpha = 0.3, color = NA) +
  geom_line(size = 1, color = colors_tam[5]) +
  scale_x_continuous(limits = c(-1,60), breaks = seq(0, 60, 10)) +
  scale_y_continuous(limits = c(0,1), labels = label_percent()) +
  labs(x = "Moral Injury Symptoms (MIOS)", 
       y = "Pr (Dysfunction = 0)",
       subtitle = "Hurdle part of the model") +
  theme_scatter


# Combine the different plots
plot_conditional_effects_2_combo <-
  (plot_conditional_effects_2 / plot_conditional_effects_2_mu / plot_conditional_effects_2_hu) +
  plot_layout(axes = "collect") +
  plot_annotation(title = "Conditional effects of moral injury symptoms on dysfunction",
                  subtitle = "Conditional effects at the mean of all covariates (Sample 2)",
                  theme = theme(plot.title = element_text(face = "bold"),
                                plot.subtitle = element_text()))


# Print to console
plot_conditional_effects_2_combo %>% print()

# Save the combined plots
ggsave(plot = plot_conditional_effects_2_combo, file = here::here("output/plot-conditional-effects-2.jpg"), width = 6, height = 8)

