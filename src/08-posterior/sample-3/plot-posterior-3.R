...
#   annotate(geom = "text", label = "95%", x = 1.69, y = .035, color = "#abc9c8", fontface = "bold", size = 3) +
  ...
  scale_x_continuous(breaks = seq(.5, 2, .2), limits = c(.7, 2)) +
  scale_thickness_shared()
ggsave(plot = plot_posterior_gamma_3, file = here::here("output/plot-posterior-gamma-3.jpg"), width = 6, height = 4)
...
plot_posterior_logistic_3 <-
  ...
  labs(y = NULL, 
       ...
       subtitle = "<span style = 'color:#0a3351'> **Posterior distribution of the odds rations**</span> (Survey 1)",
...
ggsave(plot = plot_posterior_logistic_3, file = here::here("output/plot-posterior-logistic-3.jpg"), width = 6, height = 4)
